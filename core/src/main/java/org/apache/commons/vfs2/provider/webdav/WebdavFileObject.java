...
public class WebdavFileObject extends HttpFileObject<WebdavFileSystem> {
            ...
            if (isDirectory(name)) {
                ...
                if (method.succeeded()) {
                    ...
                    for (final MultiStatusResponse response : responses) {
                        if (isCurrentFile(URIUtil.decode(response.getHref(), method.getResponseCharSet()), name)) {
                            continue;
                        }
    ...
    private boolean isCurrentFile(String href, URLFileName fileName) {
        ...
        // we only take into account the end of the path
